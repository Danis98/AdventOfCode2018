...
while turn <= MAX_TURNS:
    ...
    if ''.join([''.join(line) for line in n_grid]) in mem:
        cyc_start = mem[''.join([''.join(line) for line in n_grid])]
        cur = turn
        cyc_len = cur - cyc_start
        cyc_pos = (MAX_TURNS - cur) % cyc_len
        MAX_TURNS = cur + cyc_pos
    ...
    mem[''.join([''.join(line) for line in n_grid])] = turn
